...
def waliduj():
    if request.method == 'POST':
        ...
        if authenticate(login, password):
            ...
            if userid_mapping.get(str(request.form['login'])).stanowisko == "pracownik kuchni":
                return redirect(url_for("kucharz"))
            elif userid_mapping.get(str(request.form['login'])).stanowisko == 'klient':
                return redirect(url_for("klient"))
            elif userid_mapping.get(str(request.form['login'])).stanowisko == 'dostawca':
